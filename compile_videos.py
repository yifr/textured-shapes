...
def compile_all_videos(scene_path, args):
    n_frames_list = [2, 3, 7, 25]
    shifts = [5, 8, 11, 14, 17, 20, 23]
    motion_alignment = [True, False]
    nonconsecutive_frames = [np.random.randint(24) for i in range(5)]
    ...
    for n_frames in n_frames_list:
        for alignment in motion_alignment:
            directory_path = os.path.join(args.output_dir, f"frames={n_frames}_aligned={alignment}")
            if not os.path.exists(directory_path):
                os.makedirs(directory_path, exist_ok=True)
                
            output_path = os.path.join(directory_path, f"{movie_name}.gif")
            frames = img_paths[:n_frames]
    ...
    directory_path = os.path.join(args.output_dir, f"frames=5_consecutive=False")
    if not os.path.exists(directory_path):
        os.makedirs(directory_path, exist_ok=True)
    
    output_path = os.path.join(directory_path, f"{movie_name}.gif")
    frames = [img_paths[f] for f in nonconsecutive_frames]
    create_video(frames, output_path, np.min((len(frames), args.frame_rate)))
    ...
    for i, texture_dir in enumerate(texture_dirs):
        ...
        if len(img_paths) > 0 and not args.no_flow:
            for n_frames in n_frames_list:
                for alignment in motion_alignment:
                    directory_path = os.path.join(args.output_dir, f"frames={n_frames}_aligned={alignment}")
